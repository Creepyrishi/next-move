...
if st.button("Predict"):
    ...
    prediction = model.predict(transformed_X_test)  # One-hot encoded output
    
    # Ensure there are no all-zero predictions by taking the most probable class
    if np.all(prediction == 0):  # If the model predicts all zeros
        st.success("Final Prediction: Wjait a bit")
    else:
        #
        # Decode the predicted label back to original class names
